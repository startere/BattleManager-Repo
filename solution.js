...
function solve() {
  ...
  class ArmyUnit extends Unit {
    ...
    constructor(name, alignment, damage, health, count, speed){
      super(name, alignment);// extends means inherits, super shows the constructor the inherited value
      // it is the things here that are the properties of the class
      this.id = generateId();
  ...
  class Battlemanager {
    ...
    //the properties of casses work with constructors - not variables or constants;
    ...
    battle(attacker, defender){
      ...
      console.log(defender._count);
                    ...
                    //commander.name === query.name 
                    //|| 
                    //commander.alignment === query.alignment).sort();
...
//when mana is mana everywhere in he getter setter and constructor there is reursion because once the vale gets in the setter it also changes he getter and this is an infinite loop recursion, this is why this.mana should be this._mana meaning the mana that is going to be changed this avoids loops
 // var is more accessible than let
  /*
  var FrostNova = new Spell('Frost Nova', 95, function freeze(speedReduction){});
  console.log(FrostNova);
  
  var Lich = new Unit('Lich', 'evil');
  console.log(Lich);
  
  var Grunt = new ArmyUnit('Grunt', 'neutral', 20, 100, 2, 50);
  console.log(Grunt);
  
  var Elf = new ArmyUnit('Elf', 'good', 15, 35, 2, 90);
  console.log(Elf);
...
*/
  ...
  //var Kerrigan = Battlemanager1.getCommander('Kerrigan', 'evil', 150);
  console.log(Zeratul);
  //properties is sub parameter, so if you input a list of properties instead of parameters all will be combined under one parameter, paramteres when input are always separate so in this case options since it is alone when the input is several will reference only the first input paramater because parameters divided by , are on the same level as the inputoptions parameters while properties are on a lower lever
  // you input propertues using {} which unites them into one parameter for later processing
  // thus the whole thing {} can be called options and put in one variable with properties
  // thus {} is options and its properties are later accessed as options.property
...
battlemanager
  ...
  //Battlemanager1.addArmyUnitTo('Zera', Footman, Grunt);

  
  
  // objects are instances of classes; objects have properties like classes
  //object is a type, like variables come in types;the type of an instance is an object

  
  // if there is no get or set it is read only
  
  //koko(40);
  //console.log(koko.length);// function.length gives you the number of parameters passed to the fung
  //console.log(typeof 66);// typeof koko undefined typeof 49 number typeof typeof 'string'
}
// formal parameter means a parameter that is a variable and not a value
solve();
